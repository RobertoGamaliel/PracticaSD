...
export default class NotesList extends Component {
  ...
  componentDidMount() {
    ...
    this.getNotes().then(async () => {
      const res2 = await axios.get("https://practicasd2022.herokuapp.com/api/users");
      this.setState({ users: res2.data });
    });
  }

  async getNotes() {
    const res = await axios.get("https://practicasd2022.herokuapp.com/api/notes");
    //;
    var todas = [];
  ...
  deleteNote = async (id) => {
    await axios.delete("https://practicasd2022.herokuapp.com/api/notes/"+ id);
    this.getNotes();
  }

  editNote = async (id) => {
    await axios.post("https://practicasd2022.herokuapp.com/api/notes/" + id);
    this.getNotes();
  }
